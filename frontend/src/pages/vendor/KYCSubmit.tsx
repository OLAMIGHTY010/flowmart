<<<<<<<< HEAD:frontend/src/pages/vendor/VendorKYCSubmit.tsx
...
export default function KYCSubmitScreen() {
  ...
  const [documents, setDocuments] = useState<UploadDoc[]>(() => {
    const defaults = [
      ...
      { id: 'business_permit', title: 'Business Permit / Tax ID', subtitle: 'Upload your business permit document', status: 'upload' },
          ...
          <Card className="bg-surface p-4 sm:p-6 rounded-2xl border border-border/70 shadow-xs">
            <CardContent className="p-0 flex flex-col gap-4 sm:gap-5">
              <div className="flex items-center gap-2 border-b border-border/50 pb-2.5">
                ...
                <span className="text-sm sm:text-base font-bold text-foreground">
                  Business Permit / Tax ID
                </span>
              </div>

              {renderUploadCard(
                documents.find(d => d.id === 'business_permit')!,
                uploadingId,
                handleCardClick,
                handleRemoveFile
              )}
...
>>>>>>>> origin/main:frontend/src/pages/vendor/KYCSubmit.tsx
