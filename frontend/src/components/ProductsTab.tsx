...
import PromoteProductModal from '@/components/vendor/PromoteProductModal';
...
export default function ProductsTab() {
  ...
  const [promotingProduct, setPromotingProduct] = useState<any | null>(null);
  ...
  const openEditModal = (prod: any) => {
    ...
    setIsNegotiable(prod.isNegotiable || false);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-background font-body pb-24">
                ...
                <div className="flex flex-col items-end gap-2.5">
                  <div className="flex gap-2 items-center">
                    {!p.isSponsored && (
                      <button 
                        onClick={() => {
                          setPromotingProduct(p);
                          setIsPromoteModalOpen(true);
                        }}
                        className="px-2 py-1 bg-amber-100 text-amber-700 text-[10px] font-bold uppercase rounded-lg hover:bg-amber-200 transition-colors cursor-pointer border border-amber-200"
                      >
                        Promote
                      </button>
                    )}
                    {p.isSponsored && (
                      <span className="px-2 py-1 bg-amber-400 text-black text-[10px] font-bold uppercase rounded-lg shadow-sm border border-amber-500">
                        Sponsored
                      </span>
                    )}
                    <button 
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col justify-end pb-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm font-semibold text-foreground">Price Negotiable?</span>
                  </label>
                </div>
              </div>
      ...
      {isPromoteModalOpen && promotingProduct && (
        <PromoteProductModal 
