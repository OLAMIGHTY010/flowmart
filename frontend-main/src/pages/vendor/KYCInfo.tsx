...
export default function KYCInfo() {
  ...
  const [businessPermitId, setBusinessPermitId] = useState(formData.businessPermitId);
  ...
  useEffect(() => {
    updateForm({ businessName, cacNo, businessPermitId, bankName, accountNumber, accountName });
  }, [businessName, cacNo, businessPermitId, bankName, accountNumber, accountName]);
          ...
          <Card className="bg-surface p-4 sm:p-6 rounded-2xl border border-border/70 shadow-xs">
            <CardContent className="p-0 flex flex-col gap-4 sm:gap-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                ...
                <VendorInput
                  label="Business Permit ID / Tax ID"
                  placeholder="Enter Business Permit or Tax ID"
                  icon="file-text"
                  value={businessPermitId}
                  onChange={(e) => setBusinessPermitId(e.target.value)}
                  required
                />
              </div>
            </CardContent>
