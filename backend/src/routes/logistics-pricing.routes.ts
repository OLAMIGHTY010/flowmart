...
router.use(authorizeRoles('super_admin', 'admin', 'finance', 'regional_coordinator', 'area_manager'));

// Zones
router.get('/zones', LogisticsPricingController.getZones);
// Only logistics coordinators can create zones
router.post('/zones', authorizeRoles('regional_coordinator', 'area_manager'), LogisticsPricingController.createZone);
