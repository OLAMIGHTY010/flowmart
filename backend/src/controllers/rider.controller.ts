...
export const getAvailableDeliveries = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res.status(200).json({ success: true, deliveries: availableOrders });
  } catch (error) {
...
export const acceptDelivery = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!existingOrder) {
      return res.status(400).json({ success: false, message: 'Delivery no longer available or invalid' });
    }

    // Assign the rider and update status
    const [assignedOrder] = await db.update(orders)
      .set({ 
        riderId: riderId,
        status: 'assigned',
        updatedAt: new Date()
