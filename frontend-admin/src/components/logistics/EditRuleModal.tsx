...
export default function EditRuleModal({ isOpen, onClose, onSuccess, rule, zones = [] }: EditRuleModalProps) {
  ...
  useEffect(() => {
    if (rule) {
      setRuleType(rule.ruleType || 'surge');
      setZoneId(rule.zoneId || 'global');
      setValueType(rule.valueType || 'percentage');
      ...
      if (cond.trigger === 'event') {
        setConditionTrigger('event');
        setEventType(cond.eventType || 'rain');
      } else {
        setConditionTrigger('time');
        setStartTime(cond.startTime || '');
        setEndTime(cond.endTime || '');
      }
    }
  }, [rule]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.patch(
        `${API_URL}/logistics/rules/${rule.id}`, 
        { 
          ruleType, 
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto custom-scrollbar">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 sticky top-0 bg-white z-10">
          <h2 className="text-lg font-bold text-slate-800">Edit Pricing Rule</h2>
